...
.controller('GarageController', ['$rootScope','$scope','$http','$modal', function($rootScope,$scope,$http,$modal){
	...
	$scope.buildURL	  = function() {
		...
		return BASE_URL+'/api/garage/list/'+name;
	}
	...
	$scope.open = function (o, s) {
		var modalInstance = $modal.open({
			templateUrl: BASE_URL+'/assets/views/master/garageEdit.html',
	...
	$scope.remove = function(o) {
		bootbox.confirm("Apakah akan menghapus "+o.name+" ?", function(result) {
			if(result) {
				$http.get(BASE_URL+'/api/garage/delete/'+o.id).success(function(response){
	...
	$scope.trash	= function(){
		if($scope.selected.length == 0) swal('Exception', 'Belum ada data yang dipilih !');
			...
			$http.post(BASE_URL+'/api/garage/trash', $scope.selected).success(function(response){
				$scope.setPage();
			});
		}
	}
...
.controller('GarageEditController', ['$scope', '$http', '$modalInstance', 'data', function($scope, $http, $modalInstance, data){
	var original 		= data;
	...
	$scope.save = function (o) {
		if(angular.isDefined(o.id)){
			$http.post(BASE_URL+'/api/garage/update/'+o.id, o).then(function (result) {
				var x = angular.copy(o);
				x.save = 'update';
				$modalInstance.close(x);
			});
		} else{
			$http.post(BASE_URL+'/api/garage/save', o).then(function (result) {
				var x = angular.copy(o);
				x.save = 'insert';
				x.id = result.id;                    
				$modalInstance.close(x);
