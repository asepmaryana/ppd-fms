...
.controller('RouteLineController', ['$rootScope','$scope','$http','$modal', function($rootScope,$scope,$http,$modal){
	...
	$scope.buildURL	  = function() {
		...
		return BASE_URL+'/api/routeLine/list/'+name;
	}
	...
	$scope.open = function (o, s) {
		var modalInstance = $modal.open({
			templateUrl: BASE_URL+'/assets/views/master/routeLineEdit.html',
            controller: 'RouteLineEditController',
			size: s,
			resolve: {
				data: function() {
					return o;
	...
	$scope.remove = function(o) {
		bootbox.confirm("Apakah akan menghapus "+o.name+" ?", function(result) {
			if(result) {
				$http.get(BASE_URL+'/api/routeLine/delete/'+o.id).success(function(response){
	...
	$scope.trash	= function(){
		if($scope.selected.length == 0) swal('Exception', 'Belum ada data yang dipilih !');
			...
			$http.post(BASE_URL+'/api/routeLine/trash', $scope.selected).success(function(response){
				$scope.setPage();
			});
		}
	}
...
.controller('RouteLineEditController', ['$scope', '$http', '$modalInstance', 'data', function($scope, $http, $modalInstance, data){
	var original 		= data;
	$scope.lineTypes	= [];
	$http.get(BASE_URL+'/api/lineType/lists').success(function(data){
	...
	$scope.title = (angular.isDefined(data)) ? 'Edit Route Line '+data.code : 'New Route Line';
	...
	$scope.save = function (o) {
		if(angular.isDefined(o.id)){
			$http.post(BASE_URL+'/api/routeLine/update/'+o.id, o).then(function (result) {
				var x = angular.copy(o);
				x.save = 'update';
				$modalInstance.close(x);
			});
		} else{
			$http.post(BASE_URL+'/api/routeLine/save', o).then(function (result) {
				var x = angular.copy(o);
				x.save = 'insert';
				x.id = result.id;                    
				$modalInstance.close(x);
