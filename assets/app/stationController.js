...
.controller('StationController', ['$rootScope','$scope','$http','$modal', function($rootScope,$scope,$http,$modal){
	...
	$scope.buildURL	  = function() {
		...
		return BASE_URL+'/api/station/list/'+name;
	}
	...
	$scope.open = function (o, s) {
		var modalInstance = $modal.open({
			templateUrl: BASE_URL+'/assets/views/master/stationEdit.html',
	...
	$scope.remove = function(o) {
		bootbox.confirm("Apakah akan menghapus "+o.name+" ?", function(result) {
			if(result) {
				$http.get(BASE_URL+'/api/station/delete/'+o.id).success(function(response){
	...
	$scope.trash	= function(){
		if($scope.selected.length == 0) swal('Exception', 'Belum ada data yang dipilih !');
			...
			$http.post(BASE_URL+'/api/station/trash', $scope.selected).success(function(response){
				$scope.setPage();
			});
		}
	}
...
.controller('StationEditController', ['$scope', '$http', '$modalInstance', 'data', function($scope, $http, $modalInstance, data){
	$scope.serviceGroups	= [];
	$scope.stationDirects	= [];
	$scope.stationTypes		= [];
	$scope.routeLines		= [];
	$http.get(BASE_URL+'/api/serviceGroup/lists').success(function(data){
		$scope.serviceGroups	= data;
	});
	$http.get(BASE_URL+'/api/stationDirect/lists').success(function(data){
		$scope.stationDirects	= data;
	});
	$http.get(BASE_URL+'/api/stationType/lists').success(function(data){
		$scope.stationTypes	= data;
	});
	...
	$scope.title = (angular.isDefined(data)) ? 'Edit Station '+data.code : 'New Station';
	...
	$scope.save = function (o) {
		if(angular.isDefined(o.id)){
			$http.post(BASE_URL+'/api/station/update/'+o.id, o).then(function (result) {
				var x = angular.copy(o);
				x.save = 'update';
				$modalInstance.close(x);
			});
		} else{
			$http.post(BASE_URL+'/api/station/save', o).then(function (result) {
				var x = angular.copy(o);
				x.save = 'insert';
				x.id = result.id;                    
				$modalInstance.close(x);
